...
resource "google_bigquery_table" "price_usd_btc" {
  count = local.setup_bq ? 1 : 0

  project             = local.project_id
  dataset_id          = local.dataset_id
  table_id            = local.bq_price_usd_btc
  deletion_protection = local.deletion_protection
...
resource "google_bigquery_table_iam_member" "price_usd_btc_owner_sa" {
  count      = local.setup_bq ? 1 : 0
  project    = local.project_id
  dataset_id = local.dataset_id
  table_id   = google_bigquery_table.price_usd_btc[0].table_id
  role       = "roles/bigquery.dataOwner"
  member     = local.sa_member
}
