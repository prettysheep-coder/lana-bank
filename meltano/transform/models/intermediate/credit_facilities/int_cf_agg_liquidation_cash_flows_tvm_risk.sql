...
flatten as (
  select
    ...
    safe_multiply(sum(total_collateral) / 100000000.0, max(disbursed_liquidation_price_usd)) as max_disbursed_liquidation_cashflow,
    safe_multiply(sum(total_collateral) / 100000000.0, avg(disbursed_liquidation_price_usd)) as avg_disbursed_liquidation_cashflow,
    safe_multiply(sum(total_collateral) / 100000000.0, min(disbursed_liquidation_price_usd)) as min_disbursed_liquidation_cashflow,
