...
#[derive(es_entity::EsEvent, Debug, Clone, Serialize, Deserialize)]
...
#[derive(es_entity::EsEntity, Builder)]
#[builder(pattern = "owned", build_fn(error = "es_entity::EsEntityError"))]
pub struct Customer {
    pub id: CustomerId,
    pub email: String,
    ...
    pub(super) events: es_entity::EntityEvents<CustomerEvent>,
}
...
impl es_entity::TryFromEvents<CustomerEvent> for Customer {
    fn try_from_events(
        events: es_entity::EntityEvents<CustomerEvent>,
...
impl es_entity::IntoEvents<CustomerEvent> for NewCustomer {
    fn into_events(self) -> es_entity::EntityEvents<CustomerEvent> {
        es_entity::EntityEvents::init(
            self.id,
            [CustomerEvent::Initialized {
                id: self.id,
                email: self.email,
